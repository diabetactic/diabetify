...
htmlFiles.forEach(file => {
  ...
  content = content.replace(/<ion-icon([^>]*?)name="([^"]+)"([^>]*?)><\/ion-icon>/g, (match, before, iconName, after) => {
    fileReplacements++;
  ...
  content = content.replace(/<ion-spinner([^>]*?)name="([^"]+)"([^>]*?)><\/ion-spinner>/g, (match, before, spinnerType, after) => {
    fileReplacements++;
    // Map spinner types to appropriate Lucide loader icon
    const iconName = spinnerType === 'circular' || spinnerType === 'circles' ? 'loader-circle' : 'loader';
  ...
  content = content.replace(/<span class="material-symbols-outlined"[^>]*>([^<]+)<\/span>/g, (match, iconName) => {
    fileReplacements++;
  ...
  content = content.replace(/(<ion-button[^>]*>)\s*(<app-icon[^>]*><\/app-icon>)/g, (match, button, icon) => {
  ...
  content = content.replace(/(<ion-item[^>]*>)\s*(<app-icon[^>]*><\/app-icon>)/g, (match, item, icon) => {
  ...
  content = content.replace(/(<ion-button[^>]*fill="clear"[^>]*>)\s*(<app-icon[^>]*><\/app-icon>)\s*(<\/ion-button>)/g,
