#!/bin/bash
# Deny the next pending appointment in queue
# Usage: ./deny-next.sh [queue_placement]
...
echo -e "\033[1;34m[3/3] Denying appointment (placement: $QUEUE_PLACEMENT)...\033[0m"
HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" -X PUT "$BACKOFFICE_URL/appointments/deny/$QUEUE_PLACEMENT" \
  -H "Authorization: Bearer $TOKEN")

if [ "$HTTP_CODE" == "201" ] || [ "$HTTP_CODE" == "200" ]; then
  echo -e "\033[1;31mAppointment DENIED!\033[0m"
else
  echo -e "\033[1;31mFailed to deny (HTTP $HTTP_CODE)\033[0m"
  exit 1
fi
