...
```bash
npm run test:e2e                    # 228 tests (~5-8 min)
```

### Specific Test Suites

```bash
npm run test:e2e -- accessibility-audit.spec.ts      # 11 tests
npm run test:e2e -- --grep "@docker"                 # Docker integration only
...
```bash
npm run test:e2e -- --project=desktop-chromium       # 27 visual tests
```
...
npm run test:e2e -- --project=mobile-chromium --project=mobile-safari --project=mobile-samsung
```
