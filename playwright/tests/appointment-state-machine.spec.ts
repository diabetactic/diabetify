...
test.describe('Appointment State Machine', () => {
  ...
  test('full flow: NONE → PENDING → ACCEPTED → CREATED', async ({ page }) => {
    ...
    const pendingBadge = page.locator('.badge-warning:has-text("Pendiente"), .badge-warning:has-text("Pending")');
  ...
  test('denied flow: NONE → PENDING → DENIED', async ({ page }) => {
    ...
    await page.reload();
    await page.waitForLoadState('networkidle');
    const deniedBadge = page.locator('.badge-error:has-text("Rechazada"), .badge-error:has-text("Denied")');
